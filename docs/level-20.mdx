...
import OutOfOrderCorollary from "./level-20/out-of-order-corollary.yml";
import SuboptimalFinesse from "./level-20/suboptimal-finesse.yml";
import SuboptimalBluff from "./level-20/suboptimal-bluff.yml";
import NoInformationDoubleFinesse from "./level-20/no-information-double-finesse.yml";
import InformationNoFinesse1 from "./level-20/information-no-finesse-1.yml";
import InformationNoFinesse2 from "./level-20/information-no-finesse-2.yml";
...
- For example, in a 3-player game:
  ...
  - Alice has a globally known playable green 1 in her hand and Bob has nothing to do.
  - Alice clues number 2 to Cathy, which touches a 2 in slot 2.
  - Bob discards.
  - Normally, Cathy would interpret Alice's clue as a normal _Play Clue_ on the blue 2.
  - However, since Alice is _Occupied_ and Bob could have given the same clue, Cathy knows that it must be an _Occupied Self-Finesse_ instead.
  - Cathy blind-plays her _Finesse Position_. It is a red 1 and it successfully plays on the stacks.
  - Cathy now knows that her 2 is the connecting red 2.
...
- For example, in a 3-player game:
  ...
  - Cathy's hand is, from newest to oldest: `red 4, red 1, red 2, red 3, blue 4`
  ...
  - Cathy plays the red 1 from slot 2 (the leftmost red card that could be the red 1). She will go on to play the red 2, the red 3, and the red 4.
...
- For example, in a 3-player game:
  ...
  - Cathy has a red 1 on slot 1 (on _Finesse Position_).
...
#### Interaction Between the Out-of-Order Finesse and the 5 Color Ejection
...
- For example, in a 3-player game, this would be a _5 Color Ejection_:
  ...
  - Bob knows that this could be an _Out-of-Order Finesse_. Specifically, if Bob gives a follow-up number 5 _Fix Clue_ to Cathy, then Cathy would play the red 1 and red 2, Bob would blind-play the red 3 and the red 4, and finally Cathy would play the red 5.
  ...
  - Bob blind-plays his _Second Finesse Position_.
...
- For example, in a 3-player game:
  ...
  - Alice clues red to Cathy, touching two cards on slot 2 and slot 3.
  - Bob blind-plays an unrelated card.
  - Cathy marks her slot 2 card as the red 4. (Cathy knows that Alice performed a _Bluff_ and slot 2 was the focus of the clue.)
  - Cathy also marks her slot 3 card as the red 5. (Cathy knows that if it was a red 3, then Bob would be forced to give a _Fix Clue_.)
...
- For example, in a 3-player game:
  - Yellow 1 and blue 1 are played on the stacks.
  - Blue 3 and blue 4 have been discarded, making the remaining blue 3 and 4 critical.
  - Cathy has a red 3 on slot 1, a yellow 3 on slot 3 and the critical blue 3 on her chop.
  - Alice needs to save the blue 3.
  - Alice could clue number 3 to Cathy, which would touch 3 useful cards.
  - Alice chooses instead to clue blue to Cathy, which only touches the blue 3. This is clearly worse.
  - Bob realizes this is suboptimal. Since it's not just a _Save Clue_, it must be a _Play Clue_ on the blue 3.
  - Bob blind-plays his _Finesse Position_ as a _Suboptimal Finesse_ and it is blue 2.
  - Cathy realizes that Bob only played because it was a _Play Clue_, so she plays blue 3.

<SuboptimalFinesse />

- A _Suboptimal Bluff_ works similarly, but Bob blind-plays an unrelated card instead of the connecting card. For example, in the same 3-player game:
  - Alice clues blue to Cathy (the suboptimal clue).
  - Bob realizes this is suboptimal and should be a _Play Clue_ on blue 3, which would require blue 2.
  - Bob blind-plays his _Finesse Position_, and it is green 1 (unrelated).
  - Cathy sees that Bob played an unrelated card, so this was a _Suboptimal Bluff_.
  - Cathy knows her blue 3 is one-away-from-playable, and not the blue 4.

<SuboptimalBluff />

- Note that for a _Suboptimal Prompt, Finesse or Bluff_ to work, the clue has to be **really bad**, not just technically suboptimal in some slight way.
- For instance, if Bob had any clued 3 in his hands, Alice might want to avoid touching a duplicate 3 by _Good Touch Principle_, and Bob should not take it as a suboptimal move.

### The No-Information Finesse

- Sometimes, cards are re-clued to cause a _Finesse_ or _Bluff_. But most times, the re-clue will "fill in" or give extra information to the existing card.
- Formally, a _No-Information_ clue is defined by the following criteria:
  - The re-clue gave no additional positive information to the card.
  - The elements in the superposition of the re-clued card were the same before and after the clue sequence.
- Using a _No-Information_ clue to cause a _Finesse_ that gets a single card is okay. But since _No-Information_ clues are not very valuable, we agree that a _No-Information_ clue can never be a _Bluff_ that gets a single card. Instead, it must get two cards.
- For example, in a 3-player game:
  ...
  - Bob blind-plays red 1. If this was a _3 Bluff_, then no-one would be promised the red 2. However, since no new information was given, it must be a _No-Information Finesse_. Bob sees that Cathy does not have the red 2, so he must have the red 2, and can blind-play it on his next turn.

<NoInformationDoubleFinesse />

- Even if a clue does not "fill-in" a card, it is not necessarily a _No-Information_ clue, because of the second criteria.
- For example, in a 3-player game:
  - It is the first turn of the game.
  - Alice clues red to Cathy, touching a card on slot 3 as a _Play Clue_.
  - Bob blind-plays a blue 1.
  - Cathy marks her red card as either a red 2 or a red 3.
  - Later on in the game, red 1 is now played.
  - Alice clues red to Cathy, re-touching the card that is in a superposition of red 2 and red 3.
  - Bob blind-plays a blue 2.
  - Cathy updates the note on her red card to be exactly red 3.
  - Everyone at the table understands that this clue gave valuable information to Cathy's red card and would not be considered a "no-information" clue.

<InformationNoFinesse1 />

<InformationNoFinesse2 />

- The _No-Information_ principle can also be used to perform a _No-Information Layered Finesse_ on the next player. (The next player will know that it is a _Layered Finesse_ instead of a _Bluff_ because some other more-useful clue could have been given to initiate the _Bluff_.)
- A _No-Information_ clue can only cause a _No-Information Finesse_ (or a _No-Information Bluff_) if the touched card is _one-or-more-away-from-playable_.

### The No-Information Double Bluff (NIDB)

- Usually, a _No-Information_ clue conveys a _Finesse_. But you can also use it to perform a _Double Bluff_. For example, in a 3-player game:
  - Nothing is played on the stacks.
  - Alice clues 3's to Cathy, which touches one 3, the red 3.
  - Bob blind-plays blue 1. At this point, everyone knows that Alice has performed a _3 Bluff_, and from Cathy's perspective, her 3 can be any 3 at all.
  - Cathy discards.
  - Alice clues 3's to Cathy, which re-touches the red 3 (and gives no additional information to it).
  - Bob blind-plays green 1. If this was a _3 Bluff_, then no-one would be promised the green 2. However, since no new information was given, it must be a _No-Information Finesse_.
  ...
  - Since two cards were blind-played (that did not connect to anything), everyone now knows that the _No-Information_ clue was a _Double Bluff_ instead of a _Finesse_. From Cathy's perspective, her 3 can (still) be any 3 at all.
